...
public interface UserDAO { // USERS 테이블에 접근하는 매퍼 인터페이스
	...
	UserDTO selectById(String userId); // 아이디로 회원 한명 조회 (로그인 검증용 - 이 쿼리만 UPW를 포함)

	int checkUserId(String userId); // 아이디 중복여부 확인 (존재하면 1 이상)

	int insertUser(UserDTO user); // 회원가입 - 신규 회원 저장

	List<UserDTO> selectMemberList(PageRequestDTO pageRequestDTO); // 관리자 회원목록 - 검색조건/페이징에 맞는 회원 목록 조회

	int selectMemberTotalCount(PageRequestDTO pageRequestDTO); // 관리자 회원목록 - 검색조건에 맞는 전체 회원 수 조회

	int updateMemberByAdmin(UserDTO user); // 관리자 회원수정 - 탈퇴/잠금/실패횟수 초기화 반영
}
